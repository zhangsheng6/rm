...
void Can_Hook(CanRxMsg *rx_message)
{
    ...
    { 
			case CAN_flabellum0_ID:  //扇形击打板ID
			{
			flabellum[0].hitted = (rx_message)->Data[0]; // 0:未击中  1：击打正确   2：击打错误
			} break;
			case CAN_flabellum1_ID:
							{
			flabellum[1].hitted = (rx_message)->Data[0]; // 0:未击中  1：击打正确   2：击打错误
			} break;
			case CAN_flabellum2_ID:
							{
			flabellum[2].hitted = (rx_message)->Data[0]; // 0:未击中  1：击打正确   2：击打错误
			} break;
			case CAN_flabellum3_ID:
							{
			flabellum[3].hitted = (rx_message)->Data[0]; // 0:未击中  1：击打正确   2：击打错误
			} break;
			case CAN_flabellum4_ID:
							{
			flabellum[4].hitted = (rx_message)->Data[0]; // 0:未击中  1：击打正确   2：击打错误
			} break;
//					{
//						 static uint8_t num = 0;
//						 num = rx_message->StdId - CAN_flabellum0_ID; 
//						 flabellum[num].hitted = (rx_message)->Data[0]; // 0:未击中  1：击打正确   2：击打错误
			...
			case CAN_MASTER_ID:
					{
						...
						for(;num1<5;num1++)
						{	 
						 flabellum[num1].ready =  ((rx_message)->Data[num1]&0x10)>>4;//5个扇形装甲板的准备状态ready
						}
					}
